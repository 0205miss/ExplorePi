...
async function getblocktimeMonth() {
  let result = await pool.query(
    `SELECT DATE_FORMAT(created_at, '%Y-%m') as x,avg(spend) as y,sum(operation) as op FROM explorepi.block group by DATE_FORMAT(created_at, '%Y-%m') order by x asc;`
  );
  result = await JSON.parse(JSON.stringify(result));
...
async function getclaimedMonth() {
  let result = await pool.query(
    `SELECT DATE_FORMAT(claimed_at, '%Y-%m') as x,count(*) as y,sum(amount) as z FROM explorepi.claimant where claimed_at is not null and status=1 group by DATE_FORMAT(claimed_at, '%Y-%m') order by x asc;`
  );
  result = await JSON.parse(JSON.stringify(result));
...
async function getclaimanthistoryMonth() {
  let result = await pool.query(
    `SELECT DATE_FORMAT(created_at, '%Y-%m') as x,count(*) as y,sum(amount) as z FROM explorepi.claimant group by DATE_FORMAT(created_at, '%Y-%m') order by x asc;`
  );
  result = await JSON.parse(JSON.stringify(result));
...
async function getactiveMonth() {
  let result = await pool.query(
    `SELECT b.x,count(b.y) as y FROM (SELECT DATE_FORMAT(created_at, '%Y-%m') as x,count(account) as y FROM explorepi.operation group by DATE_FORMAT(created_at, '%Y-%m'),account) as b group by b.x ;`
  );
  result = await JSON.parse(JSON.stringify(result));
...
async function getblockMonth() {
  let result = await pool.query(
    `SELECT DATE_FORMAT(created_at, '%Y-%m') as x,count(id) as y,sum(success) as tx,sum(operation) as op,sum(fail) as tx_fail FROM explorepi.block group by DATE_FORMAT(created_at, '%Y-%m');`
  );
  result = await JSON.parse(JSON.stringify(result));
...
async function init() {
  ...
  //console.time("getblocktimeMonth");
  await getblocktimeMonth();
  //console.timeEnd("getblocktimeMonth");
  ...
  //console.time("getactiveMonth");
  await getactiveMonth();
  //console.timeEnd("getactiveMonth");
  ...
  //console.time("getblockMonth");
  await getblockMonth();
  //console.timeEnd("getblockMonth");
  docRef.set(
    {
      timestamp: Date.now(),
    },
