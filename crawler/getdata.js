...
async function getholderrank(){
    let result = await pool.query("select case when balance >= 0 and balance <= 1 then '<=1' when balance > 1 and balance <= 10 then '<=10' when balance > 10 and balance <= 100 then '<=100' when balance > 100 and balance <= 500 then '<=500' when balance > 500 and balance <= 1000 then '<=1,000' when balance > 1000 and balance <= 5000 then '<=5,000' when balance > 5000 and balance <= 10000 then '<=10,000' when balance > 10000 and balance <= 100000 then '<=100,000' when balance > 100000 and balance <= 1000000 then '<=1,000,000' else '>1,000,000' end as 'range', count(*) as `result` from Account where Role = 'Pioneer' group by `range` order by `range`")
    result = await JSON.parse(JSON.stringify(result))
